...
function without(flags, flag) {
  return (flags | flag) ^ flag;
}

function unsafeHas(acc, flag) {
  return acc & flag;
}

function has$2(acc, flag) {
  return (acc & flag) !== 0;
}
...
var B = {
  embed: embed,
  secondAllocate: secondAllocate,
  initialAllocate: initialAllocate,
  rootScope: rootScope,
  scope: scope,
  ...
  parse: parse,
  parseWithTypeValidation: parseWithTypeValidation
};
...
function dict(value) {
  ...
  for(var idx = 0; idx < numberOfFields; ++idx){
    ...
    string = string + (fromString(field) + ": " + itemLiteral.s);
...
function reverseConvertToJsonStringOrThrow(value, schema, spaceOpt) {
  ...
  return JSON.stringify(operationFn(schema, 40)(value), null, space);
}
...
var Definition = {
  ...
  toNode: toNode,
  toEmbededItem: toEmbededItem
};

function getRitemReversed(ritem) {
  return ritem.s;
}

function getRitemPath(ritem) {
  return ritem.p;
}
...
function setItemRitem(item, ritem) {
  item.r = ritem;
}

function getItemRitem(item) {
  return item.r;
}

function getUnsafeDitemSchema(item) {
  return item.schema;
}

function getUnsafeDitemIndex(item) {
  return item.i;
}
...
var Schema = {
  ...
  getRitemReversed: getRitemReversed,
  getRitemPath: getRitemPath,
  getFullDitemPath: getFullDitemPath,
  setItemRitem: setItemRitem,
  getItemRitem: getItemRitem,
  getUnsafeDitemSchema: getUnsafeDitemSchema,
  getUnsafeDitemIndex: getUnsafeDitemIndex,
  getItemReversed: getItemReversed,
  definitionToOutput: definitionToOutput,
  objectStrictModeCheck: objectStrictModeCheck,
  proxify: proxify,
  definitionToRitem: definitionToRitem,
  builder: builder$1,
  reverse: reverse$1,
  advancedBuilder: advancedBuilder,
  advancedReverse: advancedReverse,
  shape: shape,
  nested: nested,
  object: object$1,
  tuple: tuple,
...
exports.json = json;
exports.Catch = Catch;
exports.passingTypeFilter = passingTypeFilter;
exports.$$catch = $$catch;
